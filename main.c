...
void check_attempt_and_sleep(uint8_t* attempt)
{
    period_t tmp;
    tmp.dim = dd_Sec;
...
int main(void)
{
    ...
    while (1) {
        ...
        if (mode == nrf_send_mode) { //send data to PRX
            ...
            switch (res_send) {
            case nRF_OK:
                if (*(nRF_Answer.Data + DEVICE_ANSWER_DEV_TYPE) != SENSOR_TYPE) { //Ignore response for other sensor type
                    check_attempt_and_sleep(&attempt);
                    break;
                }
                period.dim = dd_Min;
