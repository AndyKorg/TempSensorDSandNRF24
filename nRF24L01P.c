...
#define REG_ADDRESS                  \
    ...
    } //default address for registraion PTX mode
...
#define PTX_REG_TYPE_BYTE 0 //number byte type devices from PTX
#define PRX_NO_REG_MODE 0xff //prcs not in registration mode
...
uint8_t nRF_ExchangeSPI(uint8_t value)
{
    ...
    for (uint8_t i = 8; i; i--) {
        Ret <<= 1;
        if
            BitIsSet(nRF_PORTIN, nRF_SPI_MISO) //read MISO
                Ret
                |= 1;
...
/*
\brief Registers the transmitter (PTX) on the receiver (PRX).
Returns ok and the last accepted response from ptx in the response structure.
*/
nrf_err_t nRF_TransmitReg(nrf_Response_t* nRF_Resp)
{
    ...
    nrf_err_t ret = nRF_ERR_NO_REG_MODE;
    while (attempt) {
        ret = send(reg_adr, reg_buf, PTX_REG_MODE_LEN, nRF_Resp);
        if (ret == nRF_ERR_NO_MODULE)
            break;
        if (ret == nRF_OK) {
...
/*
\brief Initializes the nRF24L01 interface and transmits Data over the nRF_PIPE channel. High byte forward.
Returns nRF_ERR_ADDR_NOT_FOUND if addresse not set, or returns result from send function.
If received normally, the nRF_Resp structure is full
*/
