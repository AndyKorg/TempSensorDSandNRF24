...
#define nRF_0_ALLOWED	7							// Only '0' allowed. can be used as a flag for the presence of a module
...
#define nRF_RX_DATA_RDY(st)		(st & (1<<nRF_RX_DR))		//В буфере RX_FIFO есть новые принятые данные
#define nRF_TX_DATA_END(st)		(st & (1<<nRF_TX_DS))		//Передача успешно закончена. Если включен автоответ, то взводится только после получения ACK от приемника
#define nRF_TX_ERROR(st)		(st & (1<<nRF_MAX_RT))		//Ошибка передачи. Количество попыток повторной передачи достигнуто, а ответ так и не получен. Взведенный флаг запрещает передачи
#define nRF_TX_FIFO_FULL(st)	(st & (1<<nRF_TX_FULL_ST))	//Буфер передатчика заполнен
